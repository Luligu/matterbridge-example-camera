 ...
 *
 * SolicitOffer creates a real werift RTCPeerConnection (see {@link WeriftWebRtcSession}) per session and invokes
 * Offer (with a real generated SDP offer, not a placeholder) on a bound WebRtcTransportRequestor client, if one is
 * bound; otherwise the Offer is silently skipped. EndSession closes that peer connection. Beyond offer generation and
 * teardown, this is still a signaling-only implementation: it records the received SDP answers/offers and ICE
 * candidates but does not yet apply them to the underlying peer connection or attach an encoded media track, so no
 * RTP flow is established. ProvideOffer, in particular, never invokes Answer on the requestor that sent it —
 * completing that side of the Offer/Answer flow, and wiring ProvideAnswer/ProvideIceCandidates into the real peer
 * connection, are follow-up steps.
 *
