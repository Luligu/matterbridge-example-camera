...
### Known limitation: some Firefox clients only offer a link-local address
...
This relies on the client machine actually answering that mDNS query with its real LAN address. On some machines, Firefox only advertises a non-routable IPv6 link-local (`fe80::...`) address instead of the real LAN IP, even with `media.peerconnection.ice.obfuscate_host_addresses` disabled in `about:config` (i.e. even sending its real candidate directly instead of an mDNS name, that candidate was still just the link-local address). A link-local address can never be reached by a remote peer, so the connection stays black-screen no matter how the candidate is resolved. This has been observed with Firefox on a machine where Chromium-based browsers (Edge, Chrome) worked fine on the same network — if a stream stays black specifically with Firefox, check what real address(es) it's actually offering (e.g. via `about:webrtc`) before assuming a bug in this plugin.
