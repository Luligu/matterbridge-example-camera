...
- `camera-color-test-1920-1080.jpeg`: 1920×1080 (16:9) broadcast-style snapshot calibration card returned by the example's `CaptureSnapshot` command. It contains color bars, grayscale references, geometry targets, focus patterns, safe-area guides, and near-black/near-white patches for inspecting hue, saturation, brightness, contrast, geometry, overscan, and focus.
- `camera-color-test-1280-720.jpeg`: 1280×720 (16:9) version of the same calibration card, matching the `MATTERBRIDGE_CAMERA_WEBCAM_RESOLUTION` HD option.
- `camera-color-test-640-480.jpeg`: 640×480 (4:3) version of the same calibration card, matching the default `MATTERBRIDGE_CAMERA_WEBCAM_RESOLUTION` and the image returned by `CaptureSnapshot`.
