...
**Full-suite regression caught and fixed (2026-07-30):** `TC_WEBRTCP_2_12.py` deliberately fills the cap to exactly 5 sessions and never closes any of them — so running the full `chipTests.json` sequence, whatever test ran immediately after 2.12 inherited an already-full `currentSessions` and had its own, unrelated first session spuriously evicted. `2.13`/`2.14` are `"skip": true` so a `"reset"` on them would never fire (the skip branch in `runTests()` continues before the reset check), so this cascaded into `2.15` and, from there, `2.17`-`2.21` all failing with `"Should find session with ID N in CurrentSessions"` — none of these tests are related to capacity at all. Fixed by adding `"reset": true` to `2.15` (the first non-skipped test after `2.12`) in `chipTests.json`, clearing `currentSessions` before the rest of the chain runs. Verified: full `--test WEBRTCP` run now passes 21/26 (5 skipped), with only the pre-existing #5/#6/#9 gaps failing. **If any test between 2.12 and 2.15 is ever un-skipped or reordered, re-verify this reset is still positioned correctly** — it must land on the first test in the chain that actually executes after 2.12.
