...
**Full-suite regression caught and fixed (2026-07-30):** `TC_WEBRTCP_2_12.py` deliberately fills the cap to exactly 5 sessions and never closes any of them — so running the full `chipTests.json` sequence, whatever test ran immediately after 2.12 inherited an already-full `currentSessions` and had its own, unrelated first session spuriously evicted. `2.13`/`2.14` are `"skip": true` so a `"resetBefore"` on them would never fire (the skip branch in `runTests()` continues before the reset check), so this cascaded into `2.15` and, from there, `2.17`-`2.21` all failing with `"Should find session with ID N in CurrentSessions"` — none of these tests are related to capacity at all. Originally fixed by adding `"reset": true` to `2.15` (the first non-skipped test after `2.12`); **re-fixed (2026-07-30)** by moving it to `"resetAfter": true` on `2.12` itself once the harness gained a `resetAfter` flag (see `run-chip-tests.mjs`'s doc comment) — the fix now travels with the test that actually causes the residue, so it stays correct even if `2.13`/`2.14` are ever un-skipped or the list is reordered, instead of needing to be re-verified as "the first test that actually executes after 2.12." Verified: full `--test WEBRTCP` run now passes 21/26 (5 skipped), with only the pre-existing #5/#6/#9 gaps failing.

**Further residue found and fixed (2026-07-30), after auditing every `resetBefore`/`resetAfter` for whether it's still load-bearing:** `2.12` wasn't the only test leaving a session behind. `2.15`, `2.29`, `2.30` never call `EndSession` either, and `2.31` conditionally leaves one behind if its step 6 runs (only step 8's session gets ended in step 11). None of `2.16`-`2.27` or `2.28`-`2.32` check for an empty `CurrentSessions` precondition except `2.28` — so `2.15`'s leftover session silently survives 12 tests downstream before `2.28`'s strict "must be empty" check finally catches it (and then cascades into whatever runs after `2.28` too). Fixed by adding `"resetAfter": true` to `2.15`, `2.29`, `2.30`, and `2.31` (the actual residue-causing tests), and removing the now-unnecessary `"resetBefore": true` that had accumulated on `2.28`-`2.32` as a blunt workaround. Verified empirically (not just by reading `EndSession` call counts): removing all five `resetBefore` flags from `2.28`-`2.32` first reproduced the failure (`2.28`/`2.29` both failed with `"1 != 0 CurrentSessions must be empty!"`), confirming `2.15`'s leftover was the actual cause; adding `"resetAfter": true` to `2.15` alone (with no other changes) fixed both. Full `--test WEBRTCP` run: 21/21 pass, 10 skipped.
...
**#11 — RESOLVED. `CurrentSessions` never populated the deprecated singular `VideoStreamID`/`AudioStreamID` fields on the session struct (1 test: 2.2; discovered 2026-07-29 while verifying the #7 fix).**
`WebRTCSessionStruct` (Matter 1.6 §11.4.5.5, fields 4/5) declares `VideoStreamID`/`AudioStreamID` as optional-but-deprecated fields on each session entry, distinct from the modern `VideoStreams`/`AudioStreams` list fields (field 7+) — both exist simultaneously per spec. `webRtcTransportProviderServer.ts`'s `solicitOffer`/`provideOffer` only ever stored `videoStreams`/`audioStreams` (lists) on `this.state.currentSessions`, never the singular deprecated fields. `TC_WEBRTCP_2_2` solicits via the deprecated `videoStreamID: null`/`audioStreamID: null` request fields (step 3, correctly accepted and resolved), then reads `CurrentSessions` back (step 4) and asserts the singular `videoStreamID` field is non-null — it decoded as `NullValue` since the field was never populated, so the test failed. Not hit by 2.28/2.32 (they use the modern list fields throughout, never touching the deprecated ones). **Fixed** (2026-07-29): both `solicitOffer` and `provideOffer` now also set `videoStreamId`/`audioStreamId` on the stored session from `videoStreams[0]`/`audioStreams[0]` (or `null` if none), mirroring what `#echoDeprecatedStreamIds` already did for the command response. Purely additive — no effect on real controllers: neither real-world trace (SmartThings, python-matter-server) ever reads `CurrentSessions` back through the deprecated singular fields, and no acceptance/validation logic changed. Verified: 2.2 passes cleanly; full 54-test vitest suite for this file still green.
...
**#9 — RESOLVED. `videoStreamID`/`videoStreams` (or the audio equivalents) present simultaneously isn't rejected; test itself now `"skip": true` for an unrelated reason (1 test: 2.27).**
Matter 1.6 Application Cluster Specification §11.5.6.1.10: "If the `VideoStreams` or `AudioStreams` fields are present, and the `VideoStreamID` or `AudioStreamID` fields are present: Fail the command with `INVALID_COMMAND`." Our `#resolveStreamLists()`/`#resolveStrictStreamLists()` didn't check for this mutual-exclusion conflict — it silently preferred `request.videoStreams`/`request.audioStreams` over the deprecated single-id fields via `??`, so a request with both present just succeeded using the modern field. **Fixed** (2026-07-30) by adding `#validateNoConflictingStreamFields()`, called unconditionally (independent of `MATTERBRIDGE_STRICT_WEBRTCTRANSPORT`, since this is a basic request-shape check, not a semantic behavior change) at the top of `solicitOffer` and in `provideOffer`'s null-session branch. Verified: `TC_WEBRTCP_2_27.py` step "2a" (the step that exercises this exact conflict) now passes.

That said, `2.27` as a whole is still `"skip": true`: its step 6 unconditionally asserts `SolicitOfferResponse.deferredOffer === true`. Its own CI args (`--camera-deferred-offer`) show it's written to validate the CSA reference app's own opt-in deferred-offer/standby flow — not a general compliance requirement. Flipping our `deferredOffer` to `true` would contradict the deliberate, spec-grounded fix in Known Issue #7 below (this device has no standby/low-power state, so `false` is the accurate value). Not something further plugin code can fix without re-introducing the #7 regression.
